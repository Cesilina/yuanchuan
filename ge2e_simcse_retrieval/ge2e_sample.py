...
class DataSampler(object):
    ...
    def sample(self):
        ...
        for label in labels:
            quests = self.label2questionmap[label]
            if len(quests) >= self.m:
                fq = random.sample(quests, self.m)
                data.extend(fq)
            else:
                #数据增强获取候选文本
                i = 1 
                while i <= self.m:
                    data.append(quests[0])
                    i += 1
...
class DataLoader(object):
    ...
    数据加载器
    """
    def __init__(self, n, m, datadir, tokenizer, maxlen):
        self.n = n
        self.m = m
